# Buddy Cloud v1 Production Checklist

Buddy Cloud v1 is the default protection path for signed-in users. The app still supports browser-only use before sign-in, but signed-in budgets should be protected by encrypted Buddy Cloud backup by default.
...
- Confirm `account-delete` Edge Function is deployed for Buddy Cloud reset.
...
- Buddy Cloud v1 does not store device names, user agents, IP-derived location, or persistent device identifiers in the app database.
- The browser generates a Buddy Cloud recovery key.
...
- Local-only budgets have no recovery support after browser/device storage is lost. Buddy Cloud and exports are the supported recovery paths.

## Product Mission

- BudgetBuddy should be usable by anyone.
- Core budgeting features should remain available without paywalls.
- Buddy Cloud should make backup/sync safer and more seamless without weakening the privacy promise.

## Manual Test Matrix

- Signed-out user can keep using the app locally.
- Signed-in user is guided into Buddy Cloud protection and receives a recovery key.
- First signed-in Buddy Cloud setup uploads the local budget as an encrypted vault.
...
- Sign Out All Devices clears Buddy Cloud sync slots before Supabase global sign-out.
...
- Manual Buddy Cloud Version saves the local copy as an encrypted safety snapshot before replacing local data.
...
- Confirm Privacy Policy and Terms match current Buddy Cloud behavior.
...
- Verify no Buddy Cloud path requires Stripe Premium.
